...
const makeContext=()=>vm.createContext({document, window:{matchMedia:()=>({matches:true}),
  ...
  URL,URLSearchParams,AbortController,setTimeout:(fn,ms)=>ms===1500?(pollCallbacks.push(fn),0):setTimeout(fn,ms),clearTimeout,fetch:async(url,options)=>{
    ...
    if(url==='/api/scenarios')return {ok:true,status:200,json:async()=>({version:1,scenarios:catalog,comparison_available:comparisonAvailable,policy_workspace_available:policyWorkspaceAvailable})};
...
(async()=>{
  ...
  const sources=[{id:'source-one',title:'Conversation 01',binding:{session_sha256:'source-digest',state_sha256:'state-one'},context_status:'Saved start, notebook activity unknown.',prefix:{context:[{role:'tutor',origin:'source',text:'First source context.'}],turns:[{role:'student',origin:'generated',text:'First source question.'}]}},
    {id:'source-two',title:'Conversation 02',binding:{session_sha256:'other-digest',state_sha256:'state-two'},context_status:'A different saved start.',prefix:{context:[{role:'tutor',origin:'authored',text:'Second authored context.'}],turns:[{role:'student',origin:'generated',text:'Second <question>.'}]}}];
  ...
  assert.doesNotMatch(node('canvas').innerHTML,/Second &lt;question&gt;|First source context/);
  run("selectEvidence('turn:0')");
