...
const node = id => {
  if (!elements.has(id)) elements.set(id, {innerHTML:'', textContent:'', value:'', hidden:false,
    ...
    insertAdjacentHTML(){}, append(){}, after(){}, querySelector(){return null}, scrollIntoView(){}, hasAttribute(){return false}, focus(){document.activeElement=this}});
...
(async()=>{
  ...
  assert.match(node('canvas').innerHTML,/Notebook edits, runs and grader outcomes are unknown/);
  ...
  await run('reloadWorkspace()');
  assert.match(node('canvas').innerHTML,/Saved student decisions/);
  assert.match(node('canvas').innerHTML,/data-activity="1" aria-pressed="true"/);
  assert.match(node('canvas').innerHTML,/&lt;script&gt;help\?&lt;\/script&gt;/);
  assert.doesNotMatch(node('canvas').innerHTML,/<script>/);
  ...
  assert.match(node('canvas').innerHTML,/Tutor reply, then student chose no reply/);
