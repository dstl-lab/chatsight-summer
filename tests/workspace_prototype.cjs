...
assert.match(node('canvas').innerHTML, /data-evidence="baseline" aria-pressed="true"/);
node('inspector-toggle').onclick();
assert.equal(run('state.showInspector'), false);
assert.match(node('canvas').innerHTML, /data-evidence="baseline" aria-pressed="false"/);
...
assert.match(html, /data-evidence="step">View step details/);
run("selectEvidence('step')");
assert.equal(run('state.showInspector'), true);
assert.match(node('inspector').innerHTML, /Starter code/);
node('inspector-toggle').onclick();
assert.equal(run('state.showInspector'), false);
